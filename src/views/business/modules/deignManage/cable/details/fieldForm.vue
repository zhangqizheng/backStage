<template>
  <div class="product_edit">
    ...
    <div v-if="stepVal === 1" class="detailInfo">
      <div class="lint">
        <div>
          <span class="text">认定等级</span>
          <span class="val">{{ dictKeyMap.ORG_LEVEL[formData.authLevel] }}机构</span>
        </div>
        <div>
          <span class="text">到期时间</span>
          <span class="val">{{ formData.expTime }}</span>
        </div>
      </div>
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="联系电话">
              <span>{{ formData.gradenPlotTel }}</span>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
    ...
    <div v-else-if="stepVal === 2">
      ...
      <el-form ref="ruleForm" label-width="550px" :model="formData">
        <el-row>
          ...
          >
            <span>{{ formData.jobServiceLife }}年</span>
          </el-form-item>
        </el-row>
        <el-row>
          <el-form-item
    ...
    <div v-else-if="stepVal === 4">
      ...
      <el-form ref="ruleForm" label-width="550px" :model="formData">
        ...
        <el-row>
          <el-form-item label="期末在孵退役军人创业实体提供就业岗位数（个）">
            <span>{{ formData.finalCyJobPost }}个</span>
          </el-form-item>
        </el-row>
        <el-row>
      ...
      <div v-if="formData.gradenplotlogs && formData.gradenplotlogs.length" class="headTitle">
        历史审核结果
      </div>
      <div v-if="formData.gradenplotlogs" class="auditBox">
        <div
          v-for="(item, index) in formData.gradenplotlogs"
          :key="index"
          class="resultBox"
        >
...
<script>
import { getFieldDetail, auditField } from '../api/index'
export default {
  props: {
    title: {
      type: String,
  ...
  created() {
    getFieldDetail({ id: this.ruleForm.id }).then(res => {
      if (res.status === 0) {
    ...
    submitForm() {
      this.$refs.resultForm.validate((valid) => {
        if (valid) {
          ...
          params.gradenPlotId = this.ruleForm.gradenPlotId
          params.id = this.ruleForm.id
          params.gtype = this.ruleForm.gtype
          params.comment = this.resultMap[this.form.auditStatus]
          console.log(params)
          auditField(params).then(res => {
            if (res.status === 0) {
              this.$message({
                type: 'success',
                message: '操作成功'
